...
def setting():
    ...
    if 'loggedin' in session and session['username'] != 'admin':
        ...
        sipass = request.form['password']
        cur.execute('SELECT * FROM user WHERE username = %s AND password = %s', (nama, sipass))
        account = cur.fetchone()
        if account:
            flash("USER ALREADY EXIST")
        else:
            cur.execute("INSERT INTO user(username,password) VALUES(%s,%s)", (nama,sipass))
            mysql.connection.commit()
            cur.close()
        return redirect(url_for('setting'))
    else:
        return render_template('setting.html', username =  session['username'],data=users,msg = msg)
...
def updateUser():
    ...
    pi2 = request.form['np2']
    cur = mysql.connection.cursor()
    cur.execute("UPDATE user SET username=%s WHERE username=%s", (uname,pi1))
    cur.execute("UPDATE user SET password=%s WHERE password=%s", (passw,pi2))
    mysql.connection.commit()
    return redirect(url_for('setting'))
